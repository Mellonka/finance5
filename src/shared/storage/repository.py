...
from shared.errors.domain import NotFoundError
...
class RepositoryBase[T]:
    ...
    def apply_filters(cls, statement: Select[tuple[T]], filters: dict[str, Any]) -> Select[tuple[T]]:
        for custom_filter in cls.config.filter_handlers:
            statement = custom_filter.process_filters(
                cls.config.entity_cls, statement, filters | cls.config.default_filters
            )
    ...
    async def exists(self, **filters) -> bool:
        return await self.db_session.scalar(
            self.apply_filters(statement=select(self.config.primary_key[0]), filters=filters).exists().select()
        )  # type: ignore
