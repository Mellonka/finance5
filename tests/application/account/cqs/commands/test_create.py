...
async def test_create_account(
    check_eq, session_maker, name, description, balance, expected_balance, currency, type, tags
):
    """Тест создания аккаунта"""

    command = CreateAccountCommand(
        name=name, description=description, balance=balance, currency=currency, type=type, tags=tags
    )
    ...
    async with session_maker() as db_session:
        loaded_account = await db_session.scalar(select(Account).where(Account.name == name))
...
async def test_conflict_error(session_maker):
    """Тест конфликта при создании двух аккаунтов с одним именем"""

    with pytest.raises(ConflictError):
        command = CreateAccountCommand.model_validate({'name': uuid()})
        async with session_maker() as db_session:
